...
class Publication(object):
    ...
    def __scholar_pub(self, __data):
        ...
        # the authorinfo also contains the year
        for string in authorinfo.text.split():
            try:
                # the year should be a valid int, so we get the int in this string
                year = int(string)
                self.bib.setdefault('year', None)
                # but store the year as a str for parsing into datetime later
                self.bib['year'] = str(year)
            except ValueError:
                pass

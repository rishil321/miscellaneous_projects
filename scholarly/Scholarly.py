...
class Scholarly:
    ...
    def search_pubs_query(self, query: str):
        ...
        try:
            ...
            logging.error(exc)
            logging.error(
                "Check if you have been banned by Google Scholar. Try restarting your TOR browser/client.")
...
class _ScholarlySelenium(Scholarly):
    ...
    def _get_new_chrome_agent(self,
        ...
        """Creates a chromedriver to use Google Chrome with Selenium.
        ...
        """
        # set up the options for the chromedriver
        ...
        # comment out the bottom two options to view the chrome windows that are spawned
        ...
        while searching:
            try:
                ...
                if self._has_captcha(text):
                    self._tor_refresher()
                    self.session = self._get_new_session(self.__browser)
                else:
                    searching = False
